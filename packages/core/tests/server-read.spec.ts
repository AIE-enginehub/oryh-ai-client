...
import { assertServerRead, connectionId, createServerReadHost, OryhClientController } from '../src/index.js'
...
function setup() {
  ...
  return {
    ...
    state,
    request,
    binding: { origin: 'https://oryh.example.test', identity, signal: abort.signal, request },
  }
}
it('reuses the existing controller and typed business operations with no local credential access', async () => {
  const f = setup(),
    host = await createServerReadHost(f.binding),
    controller = new OryhClientController(host)
...
it('rejects writes, credential paths, absolute URLs, traversal and encoded paths', () => {
  for (const request of [
    ...
    { path: '/projects', body: {} },
    { path: '/auth/refresh' },
    { path: '/my/skill-bundle' },
    { path: '//elsewhere.test/projects' },
    { path: '/x/../projects' },
    { path: '/%70rojects' },
  ]) {
...
it('revocation drops an in-flight result and disallows cached result reuse', async () => {
  const f = setup(),
    host = await createServerReadHost(f.binding),
    [connection] = await host.connections()
  const previous = await host.executeProjects(connection!.id)
  let done!: () => void
  f.state.pending = new Promise<void>(resolve => {
    done = resolve
  })
