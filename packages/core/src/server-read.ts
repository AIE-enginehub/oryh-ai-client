...
export function assertServerRead(request: OryhRequest): void {
  ...
  if (url.origin !== 'https://bound.invalid' || !paths.has(url.pathname) || request.path.split('?')[0] !== url.pathname)
    throw denied()
}
...
export async function createServerReadHost(binding: ServerReadBinding): Promise<OryhClientHost> {
  ...
  if (
    ...
    !binding.identity.user.id.trim() ||
    !binding.identity.tenant.id.trim()
  )
    throw denied()
  binding.signal.throwIfAborted()
  const identity = structuredClone(binding.identity)
  const delegated: ServerBinding = {
    ...
    send: async (request, signal) => {
      ...
      return { status: 200, body: await binding.request(request, signal) }
    },
  }
