...
import { type SkillBundleService, type SkillPrincipal, type SkillSyncResult, samePrincipal } from './skill-bundle.js'
...
export function desktopSkillService(bundle: Pick<SkillBundleService, 'sync' | 'installedPrincipal'>): OryhSkillService {
  return {
    delivery: 'bundle',
    syncDescription:
      '通过 MCP 重新从 ORYH 获取并安装本会话企业的技能。用户要求更新或同步技能时调用；oryh-skill-identity 说技能与本会话企业身份不一致时，写入前也先调用。不改任何业务数据；不要自己下载技能。',
    ...
    identityContext(expected) {
      ...
      if (holder === null)
        return '本机的 ORYH 技能包没有记录属于哪个账号（尚未安装，或由旧版本安装）；按 skill 写入前先调用 oryh_skill_sync。'
