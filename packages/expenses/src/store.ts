...
import { type ExpenseDraft, expenseError } from './contracts.js'
...
export class MemoryExpenseStore implements ExpenseStore {
  ...
  async append(record: ExpenseRecord, previousRevision: number): Promise<void> {
    if ((this.records.get(record.id)?.revision ?? 0) !== previousRevision)
      throw expenseError('草稿已改变，请刷新后重试。')
...
export class EncryptedExpenseStore implements ExpenseStore {
  ...
  async list(): Promise<ExpenseRecord[]> {
    ...
    return Promise.all(
      [...latest].map(async ([id, { revision, name }]) => {
        try {
          ...
          throw expenseError('费用执行记录不完整或无法解密。为避免重复写入，已停止操作。')
        }
      }),
    )
  }
  ...
  async append(record: ExpenseRecord, previousRevision: number): Promise<void> {
    if (!/^[a-f0-9-]{36}$/u.test(record.id) || record.revision !== previousRevision + 1)
      throw expenseError('费用记录版本无效。')
    ...
    let file
    try {
      file = await open(join(this.directory, `${record.id}.${record.revision}.enc`), 'wx', 0o600)
    } catch {
      throw expenseError('另一请求已修改草稿，请刷新。')
    }
    try {
      await file.writeFile(Buffer.concat([nonce, cipher.getAuthTag(), content]))
      await file.sync()
    } finally {
      await file.close()
    }
