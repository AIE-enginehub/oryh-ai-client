...
export const recordSpecs: Record<
  RecordKind,
  { query: string; title: string; summary: string; date: string; fields: Record<string, string> }
> = {
  'sales-orders': {
    query: 'keyword',
    title: 'order_no',
    summary: 'customer_name_snapshot',
    date: 'order_date',
    fields: {
      order_no: '订单号',
      title: '订单名称',
  ...
  'inventory-item-details': {
    ...
    fields: {
      product_code: '产品编码',
      facility: '仓库',
      inventory_item_id: '库存项编号',
      item_status: '库存项状态',
      reason: '变动原因',
      quantity_on_hand_diff: '现存数量变动',
      available_to_promise_diff: '可承诺数量变动',
      description: '说明',
      sales_order_id: '销售订单编号',
...
export function recordColumns(kind: RecordKind): Record<string, string> {
  return { id: '编号', ...recordSpecs[kind].fields }
}
