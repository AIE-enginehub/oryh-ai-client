...
it('finds products beyond the first position page and merges all matching ledger pages before pagination', async () => {
  const read = vi.fn(async (path: string) => {
    ...
    if (u.pathname === '/inventory-items')
      return {
        data:
          page === 1
            ? [{ id: 'other', product_code: 'OTHER' }]
            : [
                { id: 'a', product_code: 'P-1' },
                { id: 'b', product_code: 'P-1' },
              ],
        meta: { total: 3, pages: 2 },
      }
    const id = u.searchParams.get('inventory_item_id')
    if (id === 'a')
      return { data: [{ id: `a${page}`, effective_at: `2026-01-0${page}` }], meta: { total: 2, pages: 2 } }
...
it('rejects incomplete pagination instead of presenting a partial count', async () => {
  await expect(
    inventoryProductQuery(query, async () => ({
      data: [{ id: 'a', product_code: 'P-1' }],
...
it('fails when any matching ledger query fails', async () => {
  await expect(
    inventoryProductQuery(query, async path => {
      if (path.startsWith('/inventory-items?'))
        return { data: [{ id: 'a', product_code: 'P-1' }], meta: { total: 1, pages: 1 } }
...
it('queries the union of selected product IDs with server-side position filtering', async () => {
  const read = vi.fn(async (path: string) => {
    ...
    if (u.pathname === '/inventory-items') {
      ...
      return { data: [{ id: `item-${id}`, product_id: id }], meta: { pages: 1, total: 1 } }
    }
    return {
      data: [{ id: u.searchParams.get('inventory_item_id'), effective_at: '2026-01-01' }],
