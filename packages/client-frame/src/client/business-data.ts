...
import { type BusinessText, defaultText } from './business-locales.js'
...
export function businessRows(result: OryhOperationResult, t: BusinessText = defaultText): BusinessRow[] {
  if (result.operationId === 'list-projects')
    return (result.result.data as readonly OryhProject[]).map(row => ({
      id: row.id,
      title: row.name,
      status: row.status,
      date: row.startDate ?? '',
      ...
      fields: [
        ...
        [t('text179'), row.code ?? '—'],
        [t('text76'), row.client ?? '—'],
        [t('text28'), row.startDate ?? '—'],
        [t('text180'), row.endDate ?? '—'],
      ],
    }))
  if (result.operationId === 'my-expense-claims')
    return (result.result.data as readonly OryhExpenseClaim[]).map(row => ({
      id: row.id,
      title: row.title,
      status: row.status,
      date: row.claimDate ?? '',
      secondary: row.currency,
      entityType: 'expense_claim',
      fields: [
        [t('text29'), row.claimDate ?? '—'],
        [t('text77'), row.currency],
        [t('text181'), row.submittedAt ?? t('text182')],
      ],
    }))
  return (result.result.data as readonly OryhTodo[]).map(row => ({
    id: row.id,
    title: row.title,
    status: row.status,
    date: row.dueAt?.slice(0, 10) ?? '',
    secondary: row.target?.title ?? row.entityType,
    entityType: 'todo',
    fields: [
      [t('text78'), row.target?.title ?? row.entityType],
      [t('text183'), row.dueAt ?? t('text184')],
      [t('text136'), row.description ?? '—'],
      [t('text185'), row.target?.deleted ? t('text186') : '—'],
    ],
  }))
}
...
export function filterRows(rows: readonly BusinessRow[], filter: ViewFilter): BusinessRow[] {
  ...
  return rows
    .filter(
      row =>
        (!query ||
          `${row.title} ${row.secondary} ${row.fields.map(field => field[1]).join(' ')}`
            .toLocaleLowerCase()
            .includes(query)) &&
