...
export function RowOpenHeader() {
  return (
    <th scope="col" className="row-open-cell">
...
export function BackToList({ onClick, disabled }: { onClick: () => void; disabled?: boolean }) {
  return (
    <Button appearance="subtle" icon={<IconArrowLeft size={17} />} disabled={Boolean(disabled)} onClick={onClick}>
      返回列表
    </Button>
  )
}
...
export function RefreshButton({ onClick, disabled }: { onClick: () => void; disabled?: boolean }) {
  return (
    <Button disabled={Boolean(disabled)} icon={<IconRefresh size={17} />} onClick={onClick}>
      刷新
    </Button>
  )
}
...
export function NewButton({
  ...
  return (
    <Button appearance="primary" icon={<IconPlus size={17} />} disabled={Boolean(disabled)} onClick={onClick}>
      {children}
    </Button>
  )
}
...
export function ClearConditionsButton({ onClick }: { onClick: () => void }) {
  return (
    <Button type="button" appearance="subtle" onClick={onClick}>
      清空条件
    </Button>
  )
}
...
export function ErrorNote({
  ...
  return (
    <MessageBar intent="error" role="alert">
      ...
      {onRetry ? (
        <Button disabled={Boolean(disabled)} onClick={onRetry}>
          重新加载
        </Button>
      ) : null}
    </MessageBar>
...
export function StaleNote({ onRefresh, disabled }: { onRefresh: () => void; disabled?: boolean }) {
  return (
    <MessageBar intent="warning">
      <MessageBarBody>服务端数据可能已在 Chat 中更新，页面上未保存的修改可能已经过时。</MessageBarBody>
      <Button disabled={Boolean(disabled)} onClick={onRefresh}>
        放弃修改并刷新
      </Button>
    </MessageBar>
  )
...
export function EmptyState({
  ...
  return (
    <div className="empty-state">
      <h3>{title ?? (filtered ? '没有符合条件的记录' : '当前没有记录')}</h3>
      <p>{hint ?? (filtered ? '调整或清空查询条件后再看。' : '刷新后，新的记录会显示在这里。')}</p>
      {children}
    </div>
  )
}
...
export function ListFooter({
  ...
  return (
    <footer className="table-footer">
      <span>{partial ? `${count} 条（仅本次载入）` : `共 ${count} 条`}</span>
      <div className="pagination">
        <Button size="small" disabled={Boolean(disabled) || page <= 1} onClick={() => onPage(page - 1)}>
          上一页
        </Button>
        <span>
          {page} / {pages}
        </span>
        <Button size="small" disabled={Boolean(disabled) || page >= pages} onClick={() => onPage(page + 1)}>
          下一页
        </Button>
      </div>
    </footer>
