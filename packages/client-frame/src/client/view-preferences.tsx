...
export const filterPreference = z
  .object({
    text: textPreference,
    status: textPreference,
    from: textPreference,
    to: textPreference,
    descending: z.boolean().catch(true),
  })
  .catch({ text: '', status: '', from: '', to: '', descending: true })
export const productPreference = z
  .array(z.object({ id: z.string().max(200), name: z.string().max(1000), code: z.string().max(1000) }))
  .max(50)
  .catch([])
...
export const queryFieldsPreference = z
...
export const queryValuesPreference = z
  .record(z.string().regex(/^[a-z_][a-z0-9_]{0,63}$/), z.string().max(200))
  .catch({})
...
export function createViewPreference<T>(scope: string | undefined, key: string, initial: T, schema: z.ZodType<T>) {
  const store = createSnapshotStore<T>(
    initial,
    scope === undefined ? undefined : { persist: { name: `oryh.view.v1:${scope}:${key}` } },
  )
  ...
  return {
    ...
    set: (next: SetStateAction<T>) =>
      store.set(normalize(typeof next === 'function' ? (next as (v: T) => T)(store.getSnapshot()) : next)),
  }
}
